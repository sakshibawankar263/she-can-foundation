...
const InitiativeCard = ({ title, desc, icon }) => (
  <div className="group p-8 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl transition-all duration-500 hover:border-indigo-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-indigo-500/10">
    ...
    <h3 className="text-2xl font-bold mb-4 group-hover:text-indigo-600 transition-colors">{title}</h3>
    ...
    <div className="max-w-6xl mx-auto px-6 py-20">
      ...
      <section>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <InitiativeCard icon="🎓" title="Empower Education" desc="Providing scholarships and digital literacy training." />
          <InitiativeCard icon="🏥" title="Health & Wellness" desc="Organizing free medical camps and nutrition support." />
          <InitiativeCard icon="🌱" title="Sustainable Growth" desc="Supporting grassroots women entrepreneurs with micro-grants." />
        </div>
      </section>
    </div>
  );
